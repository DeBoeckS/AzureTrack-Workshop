...
namespace RMotownFestival.Functions
{
    public static class ThumbnailFunction
    {
        ...
        public static void Run([BlobTrigger("festivalpicssdb/{name}", Connection = "BlobStorageConnection")] Stream myBlob, string name, ILogger log,
            [Blob("festivalthumbssdb/{name}", FileAccess.Write, Connection = "BlobStorageConnection")]Stream thumbnail)
        {
